...
// Add View extension for eraseToAnyView() method
extension View {
...
// TODO: Move nested extensions to file level
// TODO: Move extension TimeWhereView from line 2835 to file level
// TODO: Move extension View from line 3861 to file level
...
struct TimeStandard {
    ...
    func deviationPercentage(_ hours: Double) -> Double {
        if hours < min {
            return (min - hours) / min * 100
        } else if hours > max {
            return (hours - max) / max * 100
        } else {
            return 0
        }
    }
...
struct TimeWhereView: View {
    ...
    // 添加数据刷新时间相关变量
    @State private var lastDailyRefresh: Date?
    @State private var lastWeeklyRefresh: Date?
    @State private var lastMonthlyRefresh: Date?
    @State private var refreshTimer: Timer?
    @State private var cachedTasksForSelectedRange: [Task] = []
    ...
    let roleStandards: [RoleStandard] = [
        RoleStandard(
            type: "创业者",
            standards: [
                "睡觉": TimeStandard(min: 6, max: 7.5, priorityCoefficient: 5),
                "工作": TimeStandard(min: 10, max: 14, priorityCoefficient: 5),
                "会议": TimeStandard(min: 1, max: 3, priorityCoefficient: 3),
                "思考": TimeStandard(min: 1, max: 2, priorityCoefficient: 4),
                "摸鱼": TimeStandard(min: 0.5, max: 1, priorityCoefficient: 2),
                "运动": TimeStandard(min: 0.25, max: 0.75, priorityCoefficient: 4),
                "阅读": TimeStandard(min: 0.5, max: 1, priorityCoefficient: 3),
                "生活": TimeStandard(min: 0.5, max: 1.5, priorityCoefficient: 2)
            ],
        ...
        RoleStandard(
            type: "管理者",
            standards: [
                "睡觉": TimeStandard(min: 7, max: 8, priorityCoefficient: 5),
                "工作": TimeStandard(min: 8, max: 10, priorityCoefficient: 4),
                "会议": TimeStandard(min: 3, max: 6, priorityCoefficient: 4),
                "思考": TimeStandard(min: 0.5, max: 1, priorityCoefficient: 4),
                "摸鱼": TimeStandard(min: 0.5, max: 1, priorityCoefficient: 1),
                "运动": TimeStandard(min: 0.5, max: 1, priorityCoefficient: 3),
                "阅读": TimeStandard(min: 1, max: 2, priorityCoefficient: 3),
                "生活": TimeStandard(min: 2, max: 3, priorityCoefficient: 3)
            ],
            description: "管理者需要处理较多会议，在领导和管理中取得平衡，同时保持健康的生活方式。"
        ),
        RoleStandard(
            type: "白领",
            standards: [
                "睡觉": TimeStandard(min: 7, max: 8, priorityCoefficient: 4),
                "工作": TimeStandard(min: 6, max: 8, priorityCoefficient: 4),
                "会议": TimeStandard(min: 1, max: 2, priorityCoefficient: 2),
                "思考": TimeStandard(min: 0.25, max: 0.5, priorityCoefficient: 3),
                "摸鱼": TimeStandard(min: 0.5, max: 1.5, priorityCoefficient: 1),
                "运动": TimeStandard(min: 0.5, max: 1, priorityCoefficient: 4),
                "阅读": TimeStandard(min: 0.5, max: 1, priorityCoefficient: 2),
                "生活": TimeStandard(min: 3, max: 4, priorityCoefficient: 4)
            ],
    ...
    var currentWeeklySummary: TimeAnalysisSummary {
        ...
        // 添加周趋势分析（与上周对比）
        let weeklyComparison = calculateWeeklyComparison()
        
        // 上升趋势类型
        summary.trendingUpTypes = weeklyComparison.filter { $0.value > 0 }
            .sorted { $0.value > $1.value }
            .prefix(3)
            .map { (type: $0.key, increasePercentage: $0.value) }
        
        // 下降趋势类型
        summary.trendingDownTypes = weeklyComparison.filter { $0.value < 0 }
            .sorted { $0.value < $1.value }
            .prefix(3)
            .map { (type: $0.key, decreasePercentage: $0.value) }
        
        // 如果没有真实数据，使用模拟数据保持视图正常显示
    ...
    var currentMonthlySummary: TimeAnalysisSummary {
        ...
        // 添加月度趋势分析（与上月对比）
        let monthlyComparison = calculateMonthlyComparison()
        
        // 上升趋势类型
        summary.trendingUpTypes = monthlyComparison.filter { $0.value > 0 }
            .sorted { $0.value > $1.value }
            .prefix(3)
            .map { (type: $0.key, increasePercentage: $0.value) }
        
        // 下降趋势类型
        summary.trendingDownTypes = monthlyComparison.filter { $0.value < 0 }
            .sorted { $0.value < $1.value }
            .prefix(3)
            .map { (type: $0.key, decreasePercentage: $0.value) }
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                        ...
                        Button(action: {
                            ...
                            Text(range.rawValue)
                                ...
                                .foregroundColor(selectedRange == range ? 
                                    (themeManager.currentTheme == .classic ? Color.black : themeManager.colors.primary) : 
                                    themeManager.colors.secondaryText)
                                .padding(.vertical, 10)
                                .padding(.horizontal, 18)
                                .background(
                                    RoundedRectangle(cornerRadius: 16)
                                        .fill(selectedRange == range ? themeManager.colors.primary.opacity(0.15) : Color.gray.opacity(0.05))
                                        .shadow(color: selectedRange == range ? themeManager.colors.primary.opacity(0.3) : Color.clear, radius: 4, x: 0, y: 2)
                                )
                                .overlay(
                                    RoundedRectangle(cornerRadius: 16)
                                        .stroke(
                                            selectedRange == range ? 
                                                (themeManager.currentTheme == .classic ? Color(hex: "0C4A45") : themeManager.colors.primary) : 
                                                Color.gray.opacity(0.2), 
                                            lineWidth: selectedRange == range ? 1.5 : 1
                                        )
                                )
                        }
                        .help(getRefreshTimeTooltip(for: range))
                ...
                if displayTasksForSelectedRange.isEmpty {
                    emptyStateView
                } else {
                    mainContentView
                }
        ...
        .sheet(isPresented: $showTaskDetail) {
            if let taskType = selectedTaskType {
                TaskDetailView(taskType: taskType)
        ...
        .sheet(isPresented: $showWeeklySummary) {
            WeeklySummaryView(summary: generateWeeklySummary())
                .environmentObject(themeManager)
        }
        .sheet(isPresented: $showMonthlySummary) {
            MonthlySummaryView(summary: generateMonthlySummary())
                .environmentObject(themeManager)
        }
        .alert(isPresented: $showAlert) {
            Alert(
                title: Text("数据已生成"),
                message: Text("测试数据已成功创建，现在可以查看数据统计了。"),
    ...
    private var headerView: some View {
        VStack(spacing: 6) {
            HStack {
                ...
                Spacer(minLength: 15) // 确保标题与角色选择器之间至少有15点距离
                
                // 角色选择器移到这里 - 水平排列
                HStack(spacing: 7) { // 减小间距以适应更小的按钮
    ...
    private var mainContentView: some View {
        ZStack(alignment: .bottom) {
            VStack(spacing: 10) {
                ...
                ScrollView {
                    VStack(spacing: 0) { // 将spacing改为0，手动控制间距和分隔符
                        let taskTypes = getTaskTypes()
                        
                        ForEach(Array(taskTypes.enumerated()), id: \.element) { index, taskType in
                            VStack(spacing: 0) {
                                taskTypeCard(taskType: taskType)
                                    .padding(.horizontal, 32) // 增加任务卡片水平内边距，从24增加到32
                                    .padding(.top, 7) // 顶部保持不变
                                    .padding(.bottom, 4) // 减少底部内边距，原来是7，减少3点变为4
                                
                                // 在非最后一个任务类型后添加分隔符
                                if index < taskTypes.count - 1 {
                                    Divider()
                                        .background(themeManager.colors.secondaryText.opacity(0.15))
                                        .padding(.horizontal, 48) // 增加分隔线水平内边距，从40增加到48
                                }
                            }
                        }
                    }
                    .padding(.top, 6)
                    .padding(.bottom, 80) // 增加底部内边距，确保内容不会被底部浮动按钮遮挡
                }
                .refreshable {
                    // 手动刷新数据
                    loadTasksForCurrentRange()
            ...
            // 底部浮动按钮 - 仅在查看非今天的数据时显示
            if selectedRange != .today {
                VStack(spacing: 0) {
                    Divider()
                        .background(themeManager.colors.secondaryText.opacity(0.2))
                ...
                .background(themeManager.colors.background.opacity(0.8))
    ...
    // 任务类型卡片
    func taskTypeCard(taskType: String) -> AnyView {
        let stats = getTaskTypesStats()
    ...
    // 获取状态颜色
    private func getStatusColor(_ deviationType: DeviationType) -> Color {
        switch deviationType {
        case .excess:
            return Color.red
        case .deficient:
    ...
    // 为任务类型生成针对性建议
    private func getSuggestionForTaskType(taskType: String, deviationType: DeviationType, adjustmentMinutes: Int, terminatedCount: Int) -> String {
        var suggestion = ""
        ...
        case .excess:
            switch taskType {
            case "工作":
                suggestion = "建议: 工作时间过长，尝试番茄工作法提高效率"
            case "会议":
                suggestion = "建议: 减少会议时长，设置固定议程提高效率"
            case "睡觉":
                suggestion = "建议: 睡眠时间过长，考虑提高睡眠质量"
            case "摸鱼":
                suggestion = "建议: 摸鱼时间过多，转化为短休息提高专注"
            default:
                suggestion = "建议: 适当减少该类型的时间投入"
            }
        case .deficient:
            switch taskType {
            case "思考":
                suggestion = "建议: 增加思考时间，安排每日固定思考时段"
            case "运动":
                suggestion = "建议: 增加运动时间，尝试晨间10分钟快速运动"
            case "阅读":
                suggestion = "建议: 增加阅读时间，利用碎片时间提高知识摄入"
            case "生活":
                suggestion = "建议: 增加生活时间，保持工作与生活平衡"
            default:
                suggestion = "建议: 适当增加该类型的时间投入"
            }
        case .balanced:
            // 即使时间平衡，但如果有调整或终止问题，也给出建议
            if abs(adjustmentMinutes) > 0 || terminatedCount > 0 {
                if adjustmentMinutes > 0 {
                    suggestion = "建议: 时间经常延长，提高时间预估准确性"
                } else if adjustmentMinutes < 0 {
                    suggestion = "建议: 时间经常缩短，考虑任务难度评估"
                }
                
                if terminatedCount > 0 {
                    suggestion = "建议: 任务经常终止，注意减少干扰提高专注"
                }
            } else {
                suggestion = "当前任务时间分配合理"
            }
        }
        
        // 添加理想时间范围信息
        if let timeStandard = currentRoleStandard.getStandard(for: taskType) {
            let minHours = timeStandard.min
            let maxHours = timeStandard.max
            
            // 添加换行和理想范围信息
            suggestion += "\n(理想范围: \(String(format: "%.1f", minHours))-\(String(format: "%.1f", maxHours))小时)"
        }
    ...
    // 使用缓存数据的访问方法
    private var displayTasksForSelectedRange: [Task] {
        if !cachedTasksForSelectedRange.isEmpty {
            return cachedTasksForSelectedRange
        }
        // 首次加载或缓存为空时直接获取
    ...
    func getTaskTypesStats() -> [TaskTypeStat] {
        ...
        for type in taskTypes {
            let tasksOfThisType = displayTasksForSelectedRange.filter { task in task.title == type }
            let count = tasksOfThisType.count
            if count > 0 {
                let minutes = tasksOfThisType.reduce(0) { result, task in result + task.duration }
                let originalMinutes = tasksOfThisType.reduce(0) { result, task in result + task.originalDuration() }
    ...
    var totalTimeForSelectedRange: Int {
        return displayTasksForSelectedRange.reduce(0) { $0 + $1.duration }
    }
    ...
    func formatAdjustment(_ minutes: Int) -> String {
        ...
        if minutes > 0 {
            ...
            return String(format: "-%.1fh", abs(hours))
        }
    }
    ...
    func getUniqueTaskTypes() -> [String] {
        ...
        // 过滤出当前范围内实际存在的任务类型
        let existingTypes = Array(Set(displayTasksForSelectedRange.map { task in task.title }))
    ...
    func getTaskCountByType(_ type: String) -> Int {
        return displayTasksForSelectedRange.filter { task in task.title == type }.count
    }
    ...
    func getTaskTypes() -> [String] {
        ...
        // 计算每个任务类型的时间偏差绝对值
        var typeDeviations: [(type: String, deviation: Double)] = []
        
        for stat in stats {
            let hoursSpent = Double(stat.minutes) / 60.0 // 转换为小时
            if let timeStandard = currentRoleStandard.getStandard(for: stat.type) {
                // 获取理想时间范围
                let idealMin = timeStandard.min
                let idealMax = timeStandard.max
                
                // 计算偏差 - 如果时间在理想范围内，偏差为0
                // 如果低于最小值，计算与最小值的差距
                // 如果高于最大值，计算与最大值的差距
                var deviation: Double = 0
                if hoursSpent < idealMin {
                    deviation = abs(hoursSpent - idealMin) / idealMin
                } else if hoursSpent > idealMax {
                    deviation = abs(hoursSpent - idealMax) / idealMax
                }
                
                typeDeviations.append((stat.type, deviation))
            } else {
                // 如果没有标准，使用0偏差
                typeDeviations.append((stat.type, 0))
            }
        }
        
        // 按偏差绝对值降序排序（偏差越大排越前）
        typeDeviations.sort { $0.deviation > $1.deviation }
        
        // 返回排序后的任务类型
        return typeDeviations.map { $0.type }
    }
            ...
            HStack {
                ...
                Button(action: {
                    ...
                    .background(
                        ...
                        .clipShape(Capsule())
                        .shadow(color: themeManager.currentTheme == .classic ? 
                                   Color(hex: "0C4A45").opacity(0.3) : 
    ...
    private func generateAnalysisReport() {
        let stats = getTaskTypesStats()
        _ = Double(totalTimeForSelectedRange) / 60.0
        ...
        // 生成标题
        let title = "「\(selectedRole)」角色时间分配分析"
        ...
        if !excessiveTypes.isEmpty {
            ...
            // 根据角色和任务类型生成具体原因
            if excessiveTypes.contains("会议") && selectedRole == "管理者" {
                subjectiveReasons.append("作为管理者，可能过度依赖会议进行决策和沟通")
        ...
        if !deficientTypes.isEmpty {
            ...
            // 根据角色和任务类型生成具体原因
            if deficientTypes.contains("思考") && (selectedRole == "创业者" || selectedRole == "管理者") {
            ...
            if deficientTypes.contains("阅读") && selectedRole == "管理者" {
    ...
    // 辅助方法：获取上周任务
    private func getPreviousWeekTasks() -> [Task] {
        let calendar = Calendar.current
        let now = Date()
        let currentStartOfWeek = calendar.date(from: calendar.dateComponents([.yearForWeekOfYear, .weekOfYear], from: now))!
        let previousStartOfWeek = calendar.date(byAdding: .day, value: -7, to: currentStartOfWeek)!
        let previousEndOfWeek = currentStartOfWeek.addingTimeInterval(-1) // 上周结束时间
        
        return appViewModel.tasks.filter { task in
            if let completedAt = task.completedAt, task.isCompleted {
                return completedAt >= previousStartOfWeek && completedAt < previousEndOfWeek
            }
            return false
        }
    }
    ...
    // 辅助方法：获取上月任务
    private func getPreviousMonthTasks() -> [Task] {
        let calendar = Calendar.current
        let now = Date()
        let currentComponents = calendar.dateComponents([.year, .month], from: now)
        let currentStartOfMonth = calendar.date(from: currentComponents)!
        
        // 获取上个月的开始日期
        var previousMonthComponents = DateComponents()
        previousMonthComponents.month = -1
        let previousStartOfMonth = calendar.date(byAdding: previousMonthComponents, to: currentStartOfMonth)!
        
        // 上个月的结束日期就是本月的开始日期减去1秒
        let previousEndOfMonth = currentStartOfMonth.addingTimeInterval(-1)
        
        return appViewModel.tasks.filter { task in
            if let completedAt = task.completedAt, task.isCompleted {
                return completedAt >= previousStartOfMonth && completedAt < previousEndOfMonth
            }
            return false
        }
    }
    ...
    // 计算周对比分析
    func calculateWeeklyComparison() -> [String: Double] {
        let currentWeekTasks = getWeekTasks()
        let previousWeekTasks = getPreviousWeekTasks()
        
        var comparison: [String: Double] = [:]
        
        // 如果任一周的任务为空，直接返回空结果
        if currentWeekTasks.isEmpty || previousWeekTasks.isEmpty {
            return comparison
        }
        
        // 计算总时长对比
        let currentTotalTime = currentWeekTasks.reduce(0) { $0 + $1.duration }
        let previousTotalTime = previousWeekTasks.reduce(0) { $0 + $1.duration }
        if previousTotalTime > 0 {
            let totalTimeChange = (Double(currentTotalTime) - Double(previousTotalTime)) / Double(previousTotalTime) * 100
            comparison["totalTime"] = totalTimeChange
        }
        
        // 计算任务数量对比
        if !previousWeekTasks.isEmpty {
            let taskCountChange = (Double(currentWeekTasks.count) - Double(previousWeekTasks.count)) / Double(previousWeekTasks.count) * 100
            comparison["taskCount"] = taskCountChange
        }
        ...
        for taskType in taskTypes {
            let currentTypeTime = currentWeekTasks.filter { $0.title == taskType }.reduce(0) { $0 + $1.duration }
            let previousTypeTime = previousWeekTasks.filter { $0.title == taskType }.reduce(0) { $0 + $1.duration }
            
            if previousTypeTime > 0 {
                let typeTimeChange = (Double(currentTypeTime) - Double(previousTypeTime)) / Double(previousTypeTime) * 100
                comparison[taskType] = typeTimeChange
            }
        }
    ...
    // 计算月对比分析
    func calculateMonthlyComparison() -> [String: Double] {
        let currentMonthTasks = getMonthTasks()
        let previousMonthTasks = getPreviousMonthTasks()
        
        var comparison: [String: Double] = [:]
        
        // 如果任一月的任务为空，直接返回空结果
        if currentMonthTasks.isEmpty || previousMonthTasks.isEmpty {
            return comparison
        }
        
        // 计算总时长对比
        let currentTotalTime = currentMonthTasks.reduce(0) { $0 + $1.duration }
        let previousTotalTime = previousMonthTasks.reduce(0) { $0 + $1.duration }
        if previousTotalTime > 0 {
            let totalTimeChange = (Double(currentTotalTime) - Double(previousTotalTime)) / Double(previousTotalTime) * 100
            comparison["totalTime"] = totalTimeChange
        }
        ...
        // 计算任务数量对比
        if !previousMonthTasks.isEmpty {
            let taskCountChange = (Double(currentMonthTasks.count) - Double(previousMonthTasks.count)) / Double(previousMonthTasks.count) * 100
            comparison["taskCount"] = taskCountChange
        }
        ...
        for taskType in taskTypes {
            let currentTypeTime = currentMonthTasks.filter { $0.title == taskType }.reduce(0) { $0 + $1.duration }
            let previousTypeTime = previousMonthTasks.filter { $0.title == taskType }.reduce(0) { $0 + $1.duration }
    ...
    private var timeRangeSelector: some View {
        VStack(spacing: 4) {
            ...
            .padding(.vertical, 8)
            .padding(.horizontal, 32) // 增加时间范围选择器水平内边距，从24增加到32
        }
        .background(themeManager.colors.background)
    }
    ...
    private func checkAndRefreshData() {
        ...
        case .today:
            // 检查是否需要每小时刷新今日数据
            if let lastRefresh = lastDailyRefresh {
                let hoursSinceLastRefresh = calendar.dateComponents([.hour], from: lastRefresh, to: now).hour ?? 0
                if hoursSinceLastRefresh >= 1 {
                    print("刷新【今日】数据 - 已过\(hoursSinceLastRefresh)小时")
                    loadTasksForCurrentRange()
                    lastDailyRefresh = now
                }
            } else {
        ...
        case .week:
            ...
            if weekday == 7 && hour == 0 {
                // 检查今天是否已经刷新过
                if let lastRefresh = lastWeeklyRefresh {
                    let daysSinceLastRefresh = calendar.dateComponents([.day], from: lastRefresh, to: now).day ?? 0
                    if daysSinceLastRefresh >= 1 {
                        print("刷新【本周】数据 - 周六凌晨自动刷新")
                        loadTasksForCurrentRange()
                        lastWeeklyRefresh = now
                    }
                } else {
        ...
        case .month:
            ...
            if day == 1 && hour == 0 {
                // 检查本月是否已经刷新过
                if let lastRefresh = lastMonthlyRefresh {
                    let monthsSinceLastRefresh = calendar.dateComponents([.month], from: lastRefresh, to: now).month ?? 0
                    if monthsSinceLastRefresh >= 1 {
                        print("刷新【本月】数据 - 每月1号凌晨自动刷新")
                        loadTasksForCurrentRange()
                        lastMonthlyRefresh = now
                    }
                } else {
    ...
    // 加载当前选择范围的任务数据
    private func loadTasksForCurrentRange() {
        let oldCount = cachedTasksForSelectedRange.count
        cachedTasksForSelectedRange = tasksForSelectedRange
        let newCount = cachedTasksForSelectedRange.count
        print("数据已刷新：\(selectedRange.rawValue) - 任务数量从 \(oldCount) 变为 \(newCount)")
    }
        ...
        HStack(spacing: 16) {
            ForEach(TimeRange.allCases) { range in
                Button(action: {
                    ...
                    Text(range.rawValue)
                        .font(.system(size: 16, weight: .semibold))
                        .foregroundColor(selectedRange == range ? themeManager.colors.primary : themeManager.colors.secondaryText)
                        .padding(.vertical, 10)
                        .padding(.horizontal, 16)
                        .background(
                            RoundedRectangle(cornerRadius: 16)
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: 16)
                                .stroke(selectedRange == range ? themeManager.colors.primary : Color.gray.opacity(0.2), lineWidth: selectedRange == range ? 1.5 : 1)
                        )
                }
                .help(getRefreshTimeTooltip(for: range))
                .scaleEffect(selectedRange == range ? 1.05 : 1.0)
    ...
    // 为每种范围添加刷新时间提示
    private func getRefreshTimeTooltip(for range: TimeRange) -> String {
        switch range {
        case .today:
            return "今日数据每小时刷新一次"
        case .week:
            return "本周数据在每周六00:00刷新"
        case .month:
            return "本月数据在每月1号00:00刷新"
        }
    }
    
    // 生成周摘要
    private func generateWeeklySummary() -> TimeAnalysisSummary {
        return currentWeeklySummary
    }
    
    // 生成月摘要
    private func generateMonthlySummary() -> TimeAnalysisSummary {
    ...
    // COMMENTED OUT: extension TimeWhereView {
    // 定义为方法而非属性，避免重复声明
    /*
     // DISABLED:         private func getTaskTypesStatsForSummary() -> [String: Int] {
     // DISABLED:             var result: [String: Int] = [:]
     // DISABLED:
     ...
     if monthlyTasks {
     return "暂无足够上月数据进行全面对比分析"
     }
     
     // 如果有上升趋势的类型，显示相关建议
     if !summary.trendingUpTypes.isEmpty {
     let topType = summary.trendingUpTypes[0]
     return "本月\(topType.type)时间增加了\(String(format: "%.1f", abs(topType.increasePercentage)))%，建议关注长期任务分配平衡"
     }
     
     // 如果有下降趋势的类型，显示相关建议
     if !summary.trendingDownTypes.isEmpty {
     let topType = summary.trendingDownTypes[0]
     return "本月\(topType.type)时间减少了\(String(format: "%.1f", abs(topType.decreasePercentage)))%，考虑是否需要调整月度规划"
     }
     
     return "本月时间分配相对稳定，月度波动在合理范围内"
     } catch {
     return "数据分析出现问题，请稍后再试"
     }
     }
     }
