        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack {
                ...
                HStack(spacing: 4) {
                    ...
                    Text("时间健康度: \(healthScore)/100")
                        .font(.system(size: 12, weight: .medium))
                        .foregroundColor(.white)
                    
                    // 健康状态图标
                    Image(systemName: healthScore >= 80 ? "checkmark.circle.fill" : 
                                     healthScore >= 60 ? "exclamationmark.circle.fill" : "xmark.circle.fill")
                        .font(.system(size: 10))
                        .foregroundColor(.white)
                }
                .padding(.vertical, 3)
                ...
                .background(
                    Capsule()
                        .fill(getHealthScoreColor(calculateTimeHealthScore()))
                )
            }
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(spacing: 12) {
                        ForEach(stats, id: \.type) { stat in
                            healthTimeRow(stat: stat)
                        }
                        
                        // 调整分析摘要
                        if let taskWithMostAdjustments = getTaskWithMostAdjustments() {
                            taskAdjustmentSummary(task: taskWithMostAdjustments)
                        }
                    }
                    .padding(.horizontal, 14)
                    .padding(.vertical, 12)
                }
                .frame(maxHeight: 350) // 限制最大高度，避免页面过长
            }
        }
        .background(
            RoundedRectangle(cornerRadius: 16)
...
// 健康时间行组件 - 方案E样式
private func healthTimeRow(stat: TaskTypeStat) -> some View {
    VStack(alignment: .leading, spacing: 4) {
        // 标题行：任务类型、百分比和状态
        HStack {
            // 左侧：任务类型和图标
        ...
        HStack(spacing: 2) {
            // 填充的进度部分
            let percentage = calculatePercentage(stat)
            let progressWidth = min(percentage / 10.0, 10.0) // 最多10格，每格10%
            
            ForEach(0..<10, id: \.self) { index in
                Rectangle()
                    .fill(Double(index) < progressWidth ? getColorForTaskType(stat.type) : Color.gray.opacity(0.15))
                    .frame(height: 8)
                    .cornerRadius(2)
            }
        }
        ...
        HStack {
            ...
            Text("理想: \(String(format: "%.0f", idealRange.lowerBound))-\(String(format: "%.0f", idealRange.upperBound))%")
                .font(.system(size: 11))
                .foregroundColor(themeManager.colors.secondaryText)
            ...
            // 右侧：状态评估
            HStack(spacing: 4) {
                let status = evaluateTimeStatus(stat)
                if status != .balanced {
                    Image(systemName: status == .excess ? "arrow.up.circle.fill" : "arrow.down.circle.fill")
                        .font(.system(size: 10))
                        .foregroundColor(status == .excess ? .red.opacity(0.8) : .orange.opacity(0.8))
                }
                
                Text(getStatusText(stat))
                    .font(.system(size: 11))
                    .foregroundColor(status == .balanced ? .green.opacity(0.8) : 
                                      status == .excess ? .red.opacity(0.8) : .orange.opacity(0.8))
            }
        }
...
// 任务调整分析摘要
private func taskAdjustmentSummary(task: (type: String, adjustmentCount: Int)) -> some View {
    VStack(alignment: .leading, spacing: 4) {
        Text("任务调整分析")
            .font(.system(size: 13, weight: .medium))
            .foregroundColor(themeManager.colors.text)
        ...
        HStack(alignment: .top, spacing: 4) {
            ...
            Text("\(task.type)：频繁调整(\(task.adjustmentCount)次)，建议预留缓冲时间")
                .font(.system(size: 11))
                .foregroundColor(themeManager.colors.secondaryText)
                .lineLimit(2)
        }
        
        if let terminatedTask = getTaskWithMostTerminations(), terminatedTask.terminationCount > 0 {
            HStack(alignment: .top, spacing: 4) {
                ...
                Text("\(terminatedTask.type)：经常中断(\(terminatedTask.terminationCount)次)，建议减少环境干扰")
                    .font(.system(size: 11))
                    .foregroundColor(themeManager.colors.secondaryText)
                    .lineLimit(2)
            }
...
// 获取任务类型的理想范围
private func getIdealRangeForTaskType(_ type: String) -> (lowerBound: Double, upperBound: Double) {
    switch type {
    case "工作":
        return (30, 40)
    case "会议":
...
// 计算时间百分比
private func calculatePercentage(_ stat: TaskTypeStat) -> Double {
    let totalTime = totalTimeForSelectedRange
...
// 评估时间状态
private func evaluateTimeStatus(_ stat: TaskTypeStat) -> DeviationType {
    let percentage = calculatePercentage(stat)
    let idealRange = getIdealRangeForTaskType(stat.type)
...
// 获取状态文本
private func getStatusText(_ stat: TaskTypeStat) -> String {
    let status = evaluateTimeStatus(stat)
    ...
    case .deficient:
        let percentage = calculatePercentage(stat)
        let idealRange = getIdealRangeForTaskType(stat.type)
        let difference = idealRange.lowerBound - percentage
        
        if difference > idealRange.lowerBound * 0.5 {
            return "⚠️ 严重不足"
        } else {
            return "⚠️ 不足"
        }
...
// 判断是否需要显示建议
private func needsAdvice(_ stat: TaskTypeStat) -> Bool {
    let status = evaluateTimeStatus(stat)
    return status != .balanced || stat.adjustmentMinutes != 0 || stat.terminatedCount > 0
}

// 获取针对性建议
private func getAdviceForStat(_ stat: TaskTypeStat) -> String {
    let status = evaluateTimeStatus(stat)
    
    // 优先检查调整和终止情况
    if stat.terminatedCount > 1 {
        return "此类任务频繁终止，可能存在干扰或难度评估不准确"
    }
    
    if abs(stat.adjustmentMinutes) > stat.minutes * 0.2 {
        if stat.adjustmentMinutes > 0 {
            return "任务时长经常延长，说明低估了复杂度，建议预留缓冲时间"
        } else {
            return "任务时长经常缩短，建议更精确地设置预期时间"
        }
    }
    ...
    switch status {
    case .balanced:
        return "当前时间分配合理，保持良好状态"
    case .excess:
        switch stat.type {
        case "工作":
            return "工作时间过多可能影响效率和其他活动，建议适当分配时间"
        case "会议":
            return "会议占用时间过多，考虑精简会议或转为其他沟通方式"
        case "睡觉":
            return "睡眠时间过长，可能影响日间活动效率，注意睡眠质量"
        default:
            return "时间占比高于理想范围，可适当调整"
        }
    case .deficient:
        switch stat.type {
        case "运动":
            return "运动时间不足可能影响健康，建议增加日常活动量"
        case "思考":
            return "缺少思考时间可能影响决策质量，建议预留专注思考时段"
        case "阅读":
            return "阅读时间不足可能限制知识获取，建议固定阅读时间"
        default:
            return "时间占比低于理想范围，可考虑适当增加"
        }
    }
}

// 计算时间健康评分 (0-100)
private func calculateTimeHealthScore() -> Int {
    let stats = getTaskTypesStats()
    ...
    for stat in stats {
        ...
        if percentage >= idealRange.lowerBound && percentage <= idealRange.upperBound {
            ...
            // 低于理想范围，根据差距减分
            let deficit = idealRange.lowerBound - percentage
            let percentage = deficit / idealRange.lowerBound
            itemScore = 100 - Int(percentage * 100)
            itemScore = max(itemScore, 0)
        } else {
            // 高于理想范围，根据超出减分
            let excess = percentage - idealRange.upperBound
            let percentage = excess / idealRange.upperBound
            itemScore = 100 - Int(percentage * 100)
            itemScore = max(itemScore, 0)
        }
        
        // 根据任务类型的重要性加权
        let weight = getTaskTypeWeight(stat.type)
        totalScore += itemScore * weight
        itemCount += weight
    }
    
    // 计算加权平均分
    let averageScore = itemCount > 0 ? totalScore / itemCount : 0
    
    // 调整分析扣分
    var adjustmentPenalty = 0
    if let taskWithMostAdjustments = getTaskWithMostAdjustments() {
        adjustmentPenalty = min(taskWithMostAdjustments.adjustmentCount * 2, 10)
    }
    
    // 终止分析扣分
    var terminationPenalty = 0
    if let taskWithMostTerminations = getTaskWithMostTerminations() {
        terminationPenalty = min(taskWithMostTerminations.terminationCount * 3, 15)
    }
    
    // 最终得分
    return max(0, min(100, averageScore - adjustmentPenalty - terminationPenalty))
}

// 获取任务类型权重 (用于加权计算健康分)
private func getTaskTypeWeight(_ type: String) -> Int {
    switch type {
    case "工作", "睡觉":
        return 5 // 最重要
    case "运动", "思考":
...
private func getHealthScoreColor(_ score: Int) -> Color {
    if score >= 80 {
        return Color.green.opacity(0.8) // 良好
    } else if score >= 60 {
        return Color.orange.opacity(0.8) // 一般
    } else {
        return Color.red.opacity(0.8) // 不佳
    }
}

// 获取调整次数最多的任务
private func getTaskWithMostAdjustments() -> (type: String, adjustmentCount: Int)? {
    let stats = getTaskTypesStats()
    let tasksWithAdjustments = stats.map { stat -> (type: String, adjustmentCount: Int) in
        let tasksOfThisType = tasksForSelectedRange.filter { $0.title == stat.type }
        let adjustedTasks = tasksOfThisType.filter { !$0.timeAdjustments.isEmpty }
        return (stat.type, adjustedTasks.count)
    }
    
    return tasksWithAdjustments.max { $0.adjustmentCount < $1.adjustmentCount }
}

// 获取终止次数最多的任务
private func getTaskWithMostTerminations() -> (type: String, terminationCount: Int)? {
    let stats = getTaskTypesStats()
    let tasksWithTerminations = stats.map { stat -> (type: String, terminationCount: Int) in
        let tasksOfThisType = tasksForSelectedRange.filter { $0.title == stat.type }
        let terminatedTasks = tasksOfThisType.filter { $0.isTerminated }
        return (stat.type, terminatedTasks.count)
    }
    
    return tasksWithTerminations.max { $0.terminationCount < $1.terminationCount }
}
